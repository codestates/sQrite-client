...
function Nav(props) {
    ...
    return (
        <div className="px-8 max-w-2xl mx-auto flex justify-between items-center relative py-6">
            {isLogin === true ?
                <>
                    <Link to="/"><img className="w-5/12" src={sqriteLogo} /></Link>
                ...
                <>
                    <Link to="/"><img className="w-5/12" src={sqriteLogo} /></Link>
                    <Link to="/sign" className="border-sqrite-green border-2 rounded-xl p-1 text-sqrite-green font-bold mr-10 hover:bg-sqrite-green hover:text-white">LOGIN</Link>
                </>
            }
        </div>
    )
